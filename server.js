...
app.use(session({
    secret: 'AIzaSyA0Lv37ztH_Iaz3vZvmYnn72ZqfroCCvFI', // IMPORTANT: Change this to a long, random string
    resave: false,
...
app.post('/login', async (req, res) => {
    ...
    try {
        const query = 'SELECT id_sistema, nombre, rol FROM public.personal WHERE (nombre = $1 OR dni = $1) AND clave = $2';
...
app.get('/api/obras', checkAuth, async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT id_sistema, obra FROM public.obras WHERE parent_id IS NULL OR parent_id = 0');
        res.json(result.rows);
    } catch (error) {
...
app.get('/api/subobras/:obraId', checkAuth, async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT id_sistema, obra FROM public.obras WHERE parent_id = $1', [obraId]);
        res.json(result.rows);
    } catch (error) {
...
app.get('/api/sectores', checkAuth, async (req, res) => {
    try {
        const result = await pool.query('SELECT id_sistema, sector FROM public.sectores');
        res.json(result.rows);
    } catch (error) {
...
app.post('/api/solicitar', checkAuth, async (req, res) => {
    ...
    try {
        ...
        const obraResult = await pool.query('SELECT parent_id FROM public.obras WHERE id_sistema = $1', [obraId]);
